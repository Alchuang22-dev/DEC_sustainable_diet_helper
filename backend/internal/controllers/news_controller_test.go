...
func TestDislikeNews(t *testing.T) {
    db := setupNewsTestDB()
    db.AutoMigrate(&models.News{}, &models.User{})
    ...
    // 创建用户
    user := models.User{
        OpenID:   "OpenID_DislikeNews_User",
        Nickname: "DislikeNewsUser",
    }
    db.Create(&user)

    // 创建新闻
    newsItem := models.News{
        Title:        "Dislikable News",
        DislikeCount: 3,
    }
    db.Create(&newsItem)
    ...
    tests := []struct {
        ...
        },
        {
            ...
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Query().Before("gorm:query").Register("force_find_news_err_dislike", func(tx *gorm.DB) {
                    if tx.Statement.Table == "news" {
                        tx.Error = fmt.Errorf("forced find news error")
                    }
                })
        ...
        },
        {
            ...
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Query().Remove("force_find_news_err_dislike")
                // mock user 查询失败
                db.Callback().Query().Before("gorm:query").Register("force_find_user_err_dislike", func(tx *gorm.DB) {
                    if tx.Statement.Table == "users" {
                        tx.Error = fmt.Errorf("forced find user error")
                    }
                })
        ...
        },
        {
            ...
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Query().Remove("force_find_user_err_dislike")
                // 让 user 已经点踩
                db.Model(&user).Association("DislikedNews").Append(&newsItem)
            },
            expectedStatus: http.StatusBadRequest,
            expectedError:  "You have already disliked this news",
        },
        {
            name:           "Failed To Dislike (assoc append error)",
            userID:         user.ID,
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                // 移除点踩关系
                ...
                // 模拟 association 出错
                db.Callback().Update().Before("gorm:association").Register("force_dislike_append_err", func(tx *gorm.DB) {
                    tx.Error = fmt.Errorf("forced dislike association error")
                })
            },
            expectedStatus: http.StatusInternalServerError,
            expectedError:  "Failed to dislike news",
        },
        {
            name:           "Failed To Update Dislike Count",
            userID:         user.ID,
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Update().Remove("force_dislike_append_err")
                // 注入 updateColumn 出错
                db.Callback().Update().Before("gorm:update").Register("force_update_dislike_err", func(tx *gorm.DB) {
                    if tx.Statement.Table == "news" {
        ...
        },
        {
            ...
            newsID:          fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Update().Remove("force_update_dislike_err")
            },
            expectedStatus:  http.StatusOK,
            isSuccess:       true,
    ...
    for _, tc := range tests {
        t.Run(tc.name, func(t *testing.T) {
            ...
            if tc.isSuccess {
                assert.Equal(t, "News disliked successfully", resp["message"])
...
func TestViewNews(t *testing.T) {
    ...
    // 创建用户
    user := models.User{
        OpenID:   "OpenID_ViewNews_User",
        Nickname: "ViewNewsUser",
    }
    db.Create(&user)
    ...
    tests := []struct {
        ...
        },
        {
            ...
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Query().Before("gorm:query").Register("force_find_news_err_view", func(tx *gorm.DB) {
                    if tx.Statement.Table == "news" {
                        tx.Error = fmt.Errorf("forced find news error")
                    }
                })
        ...
        },
        {
            ...
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Query().Remove("force_find_news_err_view")
                // mock user 查询失败
                db.Callback().Query().Before("gorm:query").Register("force_find_user_err_view", func(tx *gorm.DB) {
                    if tx.Statement.Table == "users" {
                        tx.Error = fmt.Errorf("forced find user error")
                    }
                })
        ...
        {
            name:           "Failed To Record News View (assoc append error)",
            userID:         user.ID,
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Query().Remove("force_oldest_viewed_err")
                db.Callback().Query().Remove("force_find_user_err_view")
                db.Callback().Update().Remove("force_delete_oldest_err")
                db.Callback().Update().Before("gorm:association").Register("force_record_view_err", func(tx *gorm.DB) {
                    tx.Error = fmt.Errorf("forced record news view error")
                })
            },
            expectedStatus: http.StatusInternalServerError,
            expectedError:  "Failed to record news view",
        },
        {
            name:           "Failed To Update View Count",
            userID:         user.ID,
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Update().Remove("force_record_view_err")
                // mock updateColumn 出错
                db.Callback().Update().Before("gorm:update").Register("force_update_viewcount_err", func(tx *gorm.DB) {
                    if tx.Statement.Table == "news" {
                        tx.Error = fmt.Errorf("forced update view_count error")
                    }
                })
            },
            expectedStatus: http.StatusInternalServerError,
            expectedError:  "Failed to update view count",
        },
        {
            name:           "Success View News",
            userID:         user.ID,
            newsID:         fmt.Sprintf("%d", newsItem.ID),
            setupFunc: func() {
                db.Callback().Update().Remove("force_update_viewcount_err")
            },
            expectedStatus: http.StatusOK,
            isSuccess:      true,
        },
    ...
    for _, tc := range tests {
        t.Run(tc.name, func(t *testing.T) {
            ...
            url := fmt.Sprintf("/news/%s/view", tc.newsID)
            req, _ := http.NewRequest("POST", url, nil)
